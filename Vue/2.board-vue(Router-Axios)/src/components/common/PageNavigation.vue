...
const prop = defineProps({ currentPage: Number, totalPage: Number });
const emit = defineEmits(["pageChange"]);

const navigationSize = parseInt(import.meta.env.VITE_ARTICLE_NAVIGATION_SIZE);

const startPage = computed(() => {
  return parseInt((prop.currentPage - 1) / navigationSize) * navigationSize + 1;
});

const endPage = computed(() => {
  let lastPage =
    parseInt((prop.currentPage - 1) / navigationSize) * navigationSize + navigationSize;
  return prop.totalPage < lastPage ? prop.totalPage : lastPage;
});

const endRange = computed(() => {
  return parseInt((prop.totalPage - 1) / navigationSize) * navigationSize < prop.currentPage;
});
